This `archive` folder in the `tocdata` directory is used to save `spotted_tocs` and `done_pubs` CSV files when the user selects the `Update CSV data...` Menu Item from the `File` Menu in the `MagTOCspotter`app.
